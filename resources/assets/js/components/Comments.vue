<template>
    <div class="comments-app">
        ...
        <!-- Comments List -->
        <div class="comments" v-if="comments" v-bind:key="index" v-for="(index , comment) in commentsData">
            <!-- Comment -->
            <div v-if="!spamComments[index] || !comment.spam" class="comment">
                ...
                <div v-if="viewcomment[index]">
                    <div class="comments" v-if="comments" v-bind:key="index2" v-for="(index2, replies) in comment.replies">
            ...
            <div class="comment reply" v-if="comment.reply == 1 && !viewcomment[index]" && !spamComments[index]>
                ...
                <div class="comment-box">
                    ...
                    <div class="comment-footer">
                        ...
                        <div class="comment-actions">
                            <ul class="list">
                                <li>Total votes: {{comment.replies[comment.replies.length-1].votes}}
                                    <a v-if="!comment.replies[comment.replies.length-1].votedByUser" v-on:click="voteComment(comment.replies[comment.replies.length-1].commentid,'replycomment',index,comment.replies.length-1,'up')">Up Votes</a>
                                    <a v-if="!comment.replies[comment.replies.length-1].votedByUser" v-on:click="voteComment(comment.replies[comment.replies.length-1].commentid,'replycomment',index,comment.replies.length-1,'down')">Down Votes</a>
                                </li>
                                <li>
                                    <a v-on:click="spamComment(comment.replies[comment.replies.length-1].commentid,'replycomment',index,index2)">Spam</a>
                                </li>
                                <li>
                                    <a v-on:click="replyCommentBox(comment.replies.length-1)">Reply</a>
                                </li>
                            </ul>
                        </div>
                    </div>
    ...
    methods: {
        fetchComments() {
            this.$http.get('comments/' + this.commentUrl).then(res => {
                ...
                this.comments = 1;
                console.log(this.commentData[0].replies.length);
            });
